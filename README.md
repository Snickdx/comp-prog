...
**Important Notes:**
- The `site/` directory is completely regenerated on each build
- Source files (`manifest.json`, `offline.html`) are stored in `docs/`
- MkDocs copies these files to `site/` using the `extra_files` configuration
- The service worker is generated after MkDocs build completes
